...
async def handle_select_collection_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    active_collections[user_id] = col_id
...
async def handle_export_collection_callback(
    ...
    # Get items
    items = db.get_items_by_collection(col_id, limit=100000)
    if not items:
        await query.edit_message_text("האוסף ריק, אין מה לייצא.")
        return

    content = _generate_export_content(col[1], items)
    bio = BytesIO(content.encode('utf-8'))
    bio.name = f"Build_Collection_{col_id}_backup.txt"
